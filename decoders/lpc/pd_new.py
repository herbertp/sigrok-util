...
class Decoder(srd.Decoder):
    ...
    annotation_rows = (
        ('data-vals', 'Data', (1, 2, 3, 4, 5, 6, 7, 8, 9)),
        ('warnings', 'Warnings', (0,)),
    )
    ...
    def handle_get_size(self, lad, lad_bits):
        # LAD[3:0]: SIZE field (1 clock cycle).
        self.size = fields['SIZE'].get(lad, 'Reserved / unknown')

        # TODO: Warning/error on invalid size values.
        if 'Reserved' in self.size:
            self.putb([0, ['Invalid size (%s)' % lad_bits]])

        self.es_block = self.samplenum
        self.putb([4, ['Size: %s' % self.size]])
        self.ss_block = self.samplenum

        self.state = 'GET CHANNEL'

    def handle_get_channel(self, lad, lad_bits):
        # LAD[3:0]: CHANNEL field (1 clock cycle).
        self.channel = fields['CHANNEL'].get(lad, 'Reserved / unknown')

        # TODO: Warning/error on invalid channel values.
        if 'Reserved' in self.channel:
            self.putb([0, ['Invalid channel (%s)' % lad_bits]])

        self.es_block = self.samplenum
        self.putb([5, ['Channel: %s' % self.channel]])
        self.ss_block = self.samplenum
    ...
    def handle_get_tar(self, lad, lad_bits):
        ...
        self.es_block = self.samplenum
        self.putb([6, ['TAR, cycle %d: %s' % (self.tarcount, lad_bits)]])
        self.ss_block = self.samplenum
    ...
    def handle_get_sync(self, lad, lad_bits):
        ...
        self.es_block = self.samplenum
        self.putb([7, ['SYNC, cycle %d: %s' % (self.synccount, self.sync_val)]])
        self.ss_block = self.samplenum
    ...
    def handle_get_data(self, lad, lad_bits):
        ...
        self.es_block = self.samplenum
        self.putb([8, ['DATA: 0x%02x' % self.databyte]])
        self.ss_block = self.samplenum
    ...
    def handle_get_tar2(self, lad, lad_bits):
        ...
        self.es_block = self.samplenum
        self.putb([9, ['TAR, cycle %d: %s' % (self.tarcount, lad_bits)]])
        self.ss_block = self.samplenum
    ...
    def decode(self):
        ...
        while True:
            ...
            elif self.state == 'GET SIZE':
                self.handle_get_size(lad, lad_bits)
            elif self.state == 'GET CHANNEL':
                self.handle_get_channel(lad, lad_bits)
